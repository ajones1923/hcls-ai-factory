...
2. [the AI platform Component Mapping](#vast-ai-os-component-mapping)
...
## the AI platform Component Mapping

The Oncology Agent maps to the the AI platform architecture as follows:
...
The Oncology Agent calls 8 peer agents via `cross_modal/cross_agent.py` and
the `/integrated-assessment` endpoint for multi-agent clinical synthesis:

| Target Agent | Integration Purpose |
|-------------|-------------------|
| CAR-T Intelligence Agent | CAR-T eligibility assessment for hematologic malignancies |
| Precision Biomarker Agent | Biomarker correlation and PGx interaction checks |
| Clinical Trial Intelligence Agent | Cross-referenced trial matching and landscape analysis |
| Cardiology Intelligence Agent | Cardiotoxicity risk assessment for anthracyclines/HER2 therapies |
| Neurology Intelligence Agent | CNS metastasis evaluation and neurotoxicity monitoring |
| Pharmacogenomics (PGx) Agent | DPYD/UGT1A1 metabolizer status for chemotherapy dosing |
| Medical Imaging Intelligence Agent | Imaging-genomic correlation for treatment response |
| Single-Cell Intelligence Agent | Tumor microenvironment and immune infiltrate analysis |

### Integrated Assessment Endpoint

The `/integrated-assessment` endpoint orchestrates multi-agent workflows by:

1. Collecting the oncology agent's therapy ranking and variant interpretation
2. Requesting PGx metabolizer status from the PGx Agent
3. Querying the Cardiology Agent for cardiac risk stratification
4. Checking CAR-T eligibility via the CAR-T Agent (for eligible histologies)
5. Aggregating findings into a unified clinical assessment
