...
3. **Therapeutic Discovery Engine** -- BioNeMo/DiffDock/RDKit for
   molecular generation, docking, and lead optimization

The oncology agent coordinates with 8 peer agents via
`cross_modal/cross_agent.py` -- CAR-T, Biomarker, Clinical Trial,
Cardiology, Neurology, PGx, Imaging, and Single-Cell -- and exposes
an `/integrated-assessment` endpoint for multi-agent clinical
synthesis.
...
## 10. Pediatric Oncology Applications

### 10.1 Pediatric Cancer Coverage

The Precision Oncology Intelligence Agent supports 11 pediatric
cancer types with age-appropriate therapy ranking, dosing
adjustments, and trial matching:

- **Acute Lymphoblastic Leukemia (ALL)** -- the most common
  pediatric malignancy, with risk stratification by cytogenetics
  (Philadelphia chromosome, MLL rearrangements, hypodiploidy)
- **Neuroblastoma** -- MYCN amplification-driven risk stratification
  with targeted therapy matching (dinutuximab, crizotinib for ALK+)
- **Medulloblastoma** -- molecular subgroup classification (WNT,
  SHH, Group 3, Group 4) for risk-adapted therapy
- **Rhabdomyosarcoma** -- PAX-FOXO1 fusion status guiding risk
  classification and treatment intensity
- **Wilms Tumor (Nephroblastoma)** -- COG risk stratification with
  loss-of-heterozygosity and histology-based therapy ranking
- **Ewing Sarcoma** -- EWSR1 fusion detection and therapy matching
- **Hepatoblastoma** -- PRETEXT staging with alpha-fetoprotein
  monitoring and cisplatin-based regimen ranking
- **Retinoblastoma** -- RB1 mutation analysis for heritable versus
  sporadic classification and treatment planning
- **Pediatric AML** -- FLT3-ITD, NPM1, and CEBPA mutation-driven
  risk stratification with targeted therapy options
- **Pediatric Glioma** -- BRAF V600E and BRAF-KIAA1549 fusion
  detection for targeted therapy eligibility
- **Osteosarcoma** -- multi-agent chemotherapy ranking with
  resistance monitoring

### 10.2 BSA-Scaled Dosing

The therapy ranker applies body-surface-area (BSA) scaling for
pediatric patients, adjusting recommended doses from adult protocols
to pediatric-appropriate levels. When a patient age below 18 is
specified, the system:

1. Flags adult-only FDA approvals and identifies pediatric-specific
   alternatives
2. Applies BSA-scaled dose calculations where published pediatric
   dosing guidelines exist
3. Prioritizes COG (Children's Oncology Group) and SIOPE protocols
   over adult NCCN guidelines

### 10.3 MYCN Amplification as a Primary Biomarker

MYCN amplification is a critical prognostic biomarker in pediatric
oncology, particularly neuroblastoma. The knowledge graph encodes
MYCN-specific therapy associations, resistance patterns, and trial
eligibility criteria. The system identifies MYCN amplification from
VCF input and automatically triggers high-risk neuroblastoma
therapy ranking including immunotherapy (dinutuximab), isotretinoin
maintenance, and emerging ALK-targeted combinations.
...
`/integrated-assessment` endpoint orchestrates these calls,
collecting PGx metabolizer status, cardiac risk scores, CAR-T
eligibility, and imaging-genomic correlations into a unified report
alongside the oncology agent's own therapy ranking and variant
interpretation.
