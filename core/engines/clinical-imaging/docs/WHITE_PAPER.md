...
4. **Imaging Intelligence Agent** (:8524) -- Medical imaging AI and clinical decision support
5. **Autoimmune Intelligence Agent** -- Autoimmune disease genomics and therapy
6. **Pharmacogenomics Intelligence Agent** (:8107) -- Drug metabolism and genotype-guided dosing
7. **Clinical Trial Intelligence Agent** (:8538) -- Trial design, matching, and optimization
8. **Rare Disease Diagnostic Agent** (:8134) -- Phenotype-driven rare disease diagnosis
9. **Single-Cell Intelligence Agent** (:8540) -- Single-cell transcriptomics and TME profiling
...
The Imaging Intelligence Agent integrates with 4 peer agents for imaging-informed precision medicine:

- **Oncology Agent (:8527):** Lung-RADS 4A+ findings trigger molecular profiling for lung cancer driver mutations; suspicious masses prompt tumor staging correlation
- **Clinical Trial Agent (:8538):** Imaging-based trial eligibility using RECIST 1.1 and RANO response criteria for enrollment matching
- **Cardiology Agent (:8126):** CAD-RADS findings trigger familial hypercholesterolemia gene queries (LDLR, PCSK9, APOB) and cardiac risk integration
- **Neurology Agent:** Brain lesion detection triggers MS-specific genomic queries (HLA-DRB1, demyelination markers) and longitudinal lesion monitoring

### 11.5 Pediatric Oncology Imaging

The agent addresses pediatric-specific imaging challenges in oncology:

- **Posterior Fossa Medulloblastoma:** The most common pediatric brain tumor location. Brain MRI workflow detects posterior fossa masses with age-appropriate measurement thresholds, triggering genomic queries for medulloblastoma molecular subgroups (WNT, SHH, Group 3, Group 4).
- **ALL Staging:** CT/MRI for hepatosplenomegaly, lymphadenopathy, and CNS involvement in pediatric ALL. Age-appropriate normal organ size references prevent false-positive staging findings.
- **Response Assessment:** Pediatric RECIST adaptations for solid tumor response monitoring, with age-appropriate tumor volume change thresholds for longitudinal tracking.
- **Sedation Considerations:** Protocol recommendations account for sedation requirements in young children (< 6yr for MRI, < 3yr for CT), including motion-reduction strategies such as rapid MRI sequences and feed-and-wrap techniques for neonatal imaging.
