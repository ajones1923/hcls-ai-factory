...
### 2.1 Mapping to the AI platform

| the AI platform Component | Imaging Agent Role |
|---|---|
| **Object Storage** | Orthanc DICOM server: DICOM studies (CT, MRI, CXR), DICOM SR, DICOM SEG |
| **data engine** | Event-driven pipelines: study.complete webhook → MONAI MAP inference → post-processing → persistence |
| **Vector Database** | PostgreSQL + pgvector: 16 tables (structured findings + 384-dim embedding vectors), HNSW indexes |
| **retrieval engine** | BiomedCLIP embedding + pgvector similarity search + NIM LLM RAG-grounded clinical reports |
| **agent engine** | LangGraph reasoning agent (triage → longitudinal → population → report) + Streamlit portal |
...
| Hardware target | NVIDIA DGX Spark | GB10 GPU, 128GB unified LPDDR5x, $4,699 |
...
|---|---|---|---|
| **1 — Proof Build** | DGX Spark | $4,699 | 1-2 workflows, single user |
