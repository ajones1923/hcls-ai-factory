...
### 2.3 the AI platform Mapping

The Imaging Intelligence Agent maps to the AI platform infrastructure as follows:

| HCLS Component | the AI platform Resource | Notes |
|---|---|---|
| Milvus 2.4 vector store | the object store (NFS/S3) backing Milvus persistent volumes | `etcd_data`, `minio_data`, `milvus_data` volumes |
| BGE-small-en-v1.5 embeddings | the platform Compute (CPU) | Embedding inference on CPU, ~384-dim vectors |
| Claude Sonnet 4.6 LLM | External API (Anthropic) | Primary LLM, API key in `.env` |
| Llama-3 8B NIM | the platform Compute (GPU) | Fallback LLM, OpenAI-compatible endpoint |
| VISTA-3D / MAISI / VILA-M3 | the platform Compute (GPU) | NIM microservices, mock fallback when GPU unavailable |
| FastAPI + Streamlit | the platform Compute (CPU) | Application tier, ports 8524/8525 |
| Orthanc DICOM server | the object store (NFS) backing `orthanc_data` volume | DICOM C-STORE on 4242, REST on 8042 |
| Docker Compose orchestration | the platform Container Runtime | 13-service full stack or 6-service lite |
| Prometheus metrics | the platform Monitoring | `/metrics` endpoint on FastAPI |

the AI platform provides the unified storage layer for Milvus vector persistence, DICOM image storage, and NIM model weight caching. The GPU compute resources host the NIM inference containers while CPU resources handle the application tier.
