...
from src.models import AgentQuery, AgentResponse, CrossCollectionResult, CrossModalResult, WorkflowResult
...
class ImagingIntelligenceAgent:
    ...
    MODALITY_KEYWORDS = {
        ...
        "pathology": ["pathology", "histology", "histopathology", "whole slide", "wsi", "biopsy"],
    }

    REGION_KEYWORDS = {
        "head": ["head", "brain", "cranial", "intracranial"],
        "neck": ["neck", "cervical spine", "carotid", "thyroid"],
        "chest": ["chest", "lung", "pulmonary", "thorax", "thoracic"],
        "abdomen": ["abdomen", "liver", "kidney", "renal", "hepatic", "pancreas", "bowel", "appendix", "appendicitis"],
        "pelvis": ["pelvis", "pelvic", "prostate", "uterus", "ovary", "bladder"],
        "spine": ["spine", "spinal", "vertebral", "lumbar", "cervical"],
        "shoulder": ["shoulder", "rotator cuff", "glenohumeral", "labrum"],
    ...
    def run(self, query: AgentQuery) -> AgentResponse:
        ...
        if plan.search_strategy == "comparative":
            ...
            # Combine evidence from both sides for comprehensive answer
            combined_hits = []
            if hasattr(comp, 'evidence_a') and comp.evidence_a:
                hits_a = comp.evidence_a.hits if isinstance(comp.evidence_a, CrossCollectionResult) else comp.evidence_a
                combined_hits.extend(hits_a)
            if hasattr(comp, 'evidence_b') and comp.evidence_b:
                hits_b = comp.evidence_b.hits if isinstance(comp.evidence_b, CrossCollectionResult) else comp.evidence_b
                combined_hits.extend(hits_b)
            combined_hits.sort(key=lambda h: getattr(h, 'score', 0), reverse=True)
            evidence = CrossCollectionResult(
                query=query.question,
                hits=combined_hits,
                knowledge_context=getattr(comp, 'comparison_context', ''),
                total_collections_searched=getattr(
                    getattr(comp, 'evidence_a', None), 'total_collections_searched', 0
                ) + getattr(
                    getattr(comp, 'evidence_b', None), 'total_collections_searched', 0
                ),
                search_time_ms=getattr(comp, 'total_search_time_ms', 0.0),
            )
        else:
