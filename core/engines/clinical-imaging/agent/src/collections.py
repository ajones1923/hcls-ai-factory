...
Manages 11 imaging-specific collections + 1 read-only genomic collection.
...
Collections:
  ...
  - imaging_radiomics       — Quantitative radiomics features (PyRadiomics)

Read-only (created by core/engines/precision-intelligence):
...
RADIOMICS_FIELDS = [
    FieldSchema(
        ...
        is_primary=True,
        max_length=200,
        description="Radiomics record identifier",
    ),
    FieldSchema(
        name="embedding",
        dtype=DataType.FLOAT_VECTOR,
    ...
    FieldSchema(
        ...
        dtype=DataType.VARCHAR,
        max_length=20,
        description="Study date (YYYY-MM-DD)",
    ),
    FieldSchema(
        name="region_label",
        dtype=DataType.VARCHAR,
        max_length=100,
        description="Anatomical region name",
    ),
    FieldSchema(
        name="modality",
        dtype=DataType.VARCHAR,
    ...
    FieldSchema(
        ...
        dtype=DataType.VARCHAR,
        max_length=50,
        description="Feature class (firstorder, shape, glcm, etc.)",
    ),
    FieldSchema(
        name="feature_count",
    ...
    FieldSchema(
        name="mean_intensity",
        dtype=DataType.FLOAT,
        description="Mean intensity value from first-order statistics",
    ),
    FieldSchema(
        name="volume_mm3",
        dtype=DataType.FLOAT,
        description="Region volume in cubic millimeters",
    ),
    FieldSchema(
        name="sphericity",
        dtype=DataType.FLOAT,
        description="Shape sphericity (0-1)",
    ),
    FieldSchema(
        name="entropy",
        dtype=DataType.FLOAT,
        description="First-order entropy",
    ),
    FieldSchema(
        name="feature_summary",
        dtype=DataType.VARCHAR,
        max_length=5000,
        description="Text summary for RAG search",
    ),
]

RADIOMICS_SCHEMA = CollectionSchema(
    fields=RADIOMICS_FIELDS,
    description="Quantitative radiomics features extracted from medical images",
)
...
REPORTS_FIELDS = [
    FieldSchema(
        ...
        is_primary=True,
        max_length=200,
        description="Report chunk ID: {patient_id}_{study_date}_{granularity}",
    ),
    FieldSchema(
        name="embedding",
        dtype=DataType.FLOAT_VECTOR,
    ...
    FieldSchema(
        ...
        dtype=DataType.VARCHAR,
        max_length=500,
        description="Clinical indication / history",
    ),
    FieldSchema(
        name="findings_text",
        dtype=DataType.VARCHAR,
        max_length=5000,
        description="Findings section text",
    ),
    FieldSchema(
        name="impression_text",
        dtype=DataType.VARCHAR,
        max_length=2000,
        description="Impression section text",
    ),
    FieldSchema(
        name="findings_count",
        dtype=DataType.INT32,
    ...
    FieldSchema(
        ...
        dtype=DataType.BOOL,
        description="Whether a critical finding was detected",
    ),
    FieldSchema(
        name="comparison_date",
        dtype=DataType.VARCHAR,
        max_length=20,
        description="Date of comparison study",
    ),
    FieldSchema(
        name="granularity",
        dtype=DataType.VARCHAR,
        max_length=20,
        description="Embedding granularity: whole_report, section_findings, section_impression",
    ),
    FieldSchema(
        name="report_source",
        dtype=DataType.VARCHAR,
        max_length=100,
        description="Source of the report (e.g., orthanc, manual, hl7)",
    ),
]

REPORTS_SCHEMA = CollectionSchema(
    fields=REPORTS_FIELDS,
    description="Parsed radiology reports with multi-granularity embeddings",
)

# ── Genomic Evidence (read-only, created by core/engines/precision-intelligence) ───────
...
GENOMIC_EVIDENCE_SCHEMA = CollectionSchema(
    ...
    description="Genomic variant evidence (read-only, from core/engines/precision-intelligence)",
)
...
OWNED_COLLECTION_SCHEMAS: Dict[str, CollectionSchema] = {
    ...
    "imaging_radiomics": RADIOMICS_SCHEMA,
    "imaging_reports": REPORTS_SCHEMA,
}
...
COLLECTION_MODELS: Dict[str, type] = {
    ...
    "imaging_radiomics": None,  # Model in src.ingest.radiomics_parser.RadiomicsRecord
    "imaging_reports": None,  # Uses ParsedReport from src.models (multi-granularity ingest)
...
class ImagingCollectionManager:
    """Manages 12 Imaging Milvus collections (11 owned + 1 read-only genomic).
    ...
    def create_all_collections(
        ...
        """Create all 11 owned imaging collections.

        Does NOT create the genomic_evidence collection (it is read-only
        and managed by the core/engines/precision-intelligence).
        ...
        """
        logger.info("Creating all 11 imaging collections")
    ...
    def drop_collection(self, name: str) -> None:
        ...
        Refuses to drop genomic_evidence (read-only, owned by core/engines/precision-intelligence).
        ...
        if name == "genomic_evidence":
            logger.warning(
                ...
                "read-only collection owned by core/engines/precision-intelligence"
            )
            return
