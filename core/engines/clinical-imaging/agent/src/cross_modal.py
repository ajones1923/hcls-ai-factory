...
genomic_evidence collection populated by the core/engines/precision-intelligence,
...
Trigger rules:
    ...
    - CAD-RADS >= 3 -> cardiovascular genomics queries
    - PI-RADS >= 4 -> prostate cancer genomics queries
    - BI-RADS >= 4 -> breast cancer genomics queries
    - TI-RADS TR4, TR5 -> thyroid cancer genomics queries
    - LI-RADS LR-4, LR-5 -> hepatocellular carcinoma genomics queries
...
PROSTATE_GENOMICS_QUERIES = [
    "prostate cancer genomic markers BRCA2 HOXB13 ATM",
    "hereditary prostate cancer susceptibility genes",
    "prostate cancer molecular subtypes genomic classification",
]

BREAST_GENOMICS_QUERIES = [
    "BRCA1 BRCA2 hereditary breast cancer susceptibility pathogenic variants",
    "PALB2 partner and localizer of BRCA2 breast cancer risk variants",
    "CHEK2 checkpoint kinase 2 breast cancer moderate penetrance variants",
    "TP53 Li-Fraumeni syndrome breast cancer germline mutations",
]

THYROID_GENOMICS_QUERIES = [
    "BRAF V600E thyroid papillary carcinoma driver mutation",
    "RAS NRAS HRAS KRAS thyroid follicular neoplasm mutations",
    "RET PTC rearrangement thyroid papillary carcinoma",
    "TERT promoter mutation thyroid cancer aggressiveness",
]

LIVER_GENOMICS_QUERIES = [
    "TP53 tumor protein p53 hepatocellular carcinoma driver mutations",
    "CTNNB1 beta-catenin WNT pathway hepatocellular carcinoma mutations",
    "TERT promoter telomerase reverse transcriptase HCC early driver",
    "AXIN1 WNT pathway hepatocellular carcinoma tumor suppressor",
]
...
class CrossModalTrigger:
    ...
    def evaluate(self, workflow_result: WorkflowResult) -> Optional[CrossModalResult]:
        ...
        # Check cardiac CTA triggers
        if workflow_result.workflow_name == "ct_coronary_angiography":
            return self._evaluate_cardiac(workflow_result)

        # Check prostate MRI triggers
        if workflow_result.workflow_name == "mri_prostate_pirads":
            return self._evaluate_prostate(workflow_result)

        # Check breast imaging triggers
        if workflow_result.workflow_name == "breast_birads":
            return self._evaluate_breast(workflow_result)

        # Check thyroid ultrasound triggers
        if workflow_result.workflow_name == "thyroid_tirads":
            return self._evaluate_thyroid(workflow_result)

        # Check liver imaging triggers
        if workflow_result.workflow_name == "liver_lirads":
            return self._evaluate_liver(workflow_result)
    ...
    def _evaluate_cardiac(
        self, result: WorkflowResult
    ) -> Optional[CrossModalResult]:
        """Trigger genomic query for CAD-RADS >= 3 findings.

        Parses the classification string (e.g. "CAD-RADS 4A") and
        triggers if the category is 3, 4A, 4B, or 5.
        """
        classification = result.classification or ""

        # Check if CAD-RADS is >= 3
        high_risk_categories = ["3", "4A", "4B", "5"]
        high_risk = any(
            f"CAD-RADS {cat}" in classification for cat in high_risk_categories
        )
        if not high_risk:
            return None

        matched = next(
            (cat for cat in high_risk_categories if f"CAD-RADS {cat}" in classification),
            "3+",
        )
        trigger_reason = (
            f"CAD-RADS {matched} — significant coronary artery disease, "
            f"querying cardiovascular genomics"
        )
        ...
        return self._query_genomics(
            queries=CARDIAC_GENOMICS_QUERIES,
            trigger_reason=trigger_reason,
        )

    def _evaluate_prostate(
        self, result: WorkflowResult
    ) -> Optional[CrossModalResult]:
        """Trigger genomic query for PI-RADS >= 4 findings.

        Parses the classification string (e.g. "PI-RADS 4") and
        triggers if the score is 4 or 5.
        """
        classification = result.classification or ""

        # Check if PI-RADS is >= 4
        high_risk_scores = ["4", "5"]
        high_risk = any(
            f"PI-RADS {score}" in classification for score in high_risk_scores
        )
        if not high_risk:
            return None

        matched = next(
            (score for score in high_risk_scores if f"PI-RADS {score}" in classification),
            "4+",
        )
        trigger_reason = (
            f"PI-RADS {matched} — high suspicion prostate lesion, "
            f"querying prostate cancer genomics"
        )
        ...
        return self._query_genomics(
            queries=PROSTATE_GENOMICS_QUERIES,
            trigger_reason=trigger_reason,
        )

    def _evaluate_breast(
        self, result: WorkflowResult
    ) -> Optional[CrossModalResult]:
        """Trigger genomic query for BI-RADS >= 4 findings.

        Parses the classification string (e.g. "BI-RADS 4C") and
        triggers if the category is 4, 4A, 4B, 4C, or 5.
        """
        classification = result.classification or ""

        # Check if BI-RADS is >= 4 (order matters: check subcategories before "4")
        high_risk_categories = ["4A", "4B", "4C", "4", "5"]
        high_risk = any(
            f"BI-RADS {cat}" in classification for cat in high_risk_categories
        )
        if not high_risk:
            return None

        matched = next(
            (cat for cat in high_risk_categories if f"BI-RADS {cat}" in classification),
            "4+",
        )
        trigger_reason = (
            f"BI-RADS {matched} — suspicious breast finding, "
            f"querying breast cancer genomics"
        )
        ...
        return self._query_genomics(
            queries=BREAST_GENOMICS_QUERIES,
            trigger_reason=trigger_reason,
        )

    def _evaluate_thyroid(
        self, result: WorkflowResult
    ) -> Optional[CrossModalResult]:
        """Trigger genomic query for TI-RADS TR4 or TR5 findings.

        Parses the classification string (e.g. "TI-RADS TR5") and
        triggers if the category is TR4 or TR5.
        """
        classification = result.classification or ""

        # Check if TI-RADS is TR4 or TR5
        high_risk_categories = ["TR4", "TR5"]
        high_risk = any(
            f"TI-RADS {cat}" in classification for cat in high_risk_categories
        )
        if not high_risk:
            return None

        matched = next(
            (cat for cat in high_risk_categories if f"TI-RADS {cat}" in classification),
            "TR4+",
        )
        trigger_reason = (
            f"TI-RADS {matched} — highly suspicious thyroid nodule, "
            f"querying thyroid cancer genomics"
        )
        ...
        return self._query_genomics(
            queries=THYROID_GENOMICS_QUERIES,
            trigger_reason=trigger_reason,
        )
    ...
    def _evaluate_liver(
        ...
        """Trigger genomic query for LI-RADS LR-4 or LR-5 findings.

        Parses the classification string (e.g. "LI-RADS LR-5") and
        triggers if the category is LR-4 or LR-5.
        """
        classification = result.classification or ""

        # Check if LI-RADS is LR-4 or LR-5
        high_risk_categories = ["LR-4", "LR-5"]
        high_risk = any(
            f"LI-RADS {cat}" in classification for cat in high_risk_categories
        )
        if not high_risk:
            return None

        matched = next(
            (cat for cat in high_risk_categories if f"LI-RADS {cat}" in classification),
            "LR-4+",
        )
        trigger_reason = (
            f"LI-RADS {matched} — probable or definite hepatocellular carcinoma, "
            f"querying HCC genomics"
        )
        ...
        return self._query_genomics(
            queries=LIVER_GENOMICS_QUERIES,
            trigger_reason=trigger_reason,
        )
