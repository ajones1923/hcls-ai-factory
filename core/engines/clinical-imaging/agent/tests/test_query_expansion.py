...
Validates all 12 expansion maps (with extended pathology, body region,
and guideline entries) and the expand_query() function for synonym
resolution, case insensitivity, and multi-map coverage.
...
class TestExpandQuery:
    ...
    def test_emphysema_expansion(self):
        """Emphysema query expands to COPD terms."""
        terms = expand_query("emphysema ct findings")
        assert "copd" in terms or "air trapping" in terms

    def test_breast_mass_expansion(self):
        """Breast mass query expands to mammography terms."""
        terms = expand_query("breast mass evaluation")
        assert "bi-rads" in terms or "mammographic mass" in terms

    def test_thyroid_nodule_expansion(self):
        """Thyroid nodule query expands to TI-RADS terms."""
        terms = expand_query("thyroid nodule classification")
        assert "ti-rads" in terms or "tirads" in terms

    def test_bowel_obstruction_expansion(self):
        """Bowel obstruction query expands to SBO terms."""
        terms = expand_query("bowel obstruction ct")
        assert "sbo" in terms or "transition point" in terms

    def test_coronary_calcification_expansion(self):
        """Coronary calcification expands to Agatston terms."""
        terms = expand_query("coronary calcification scoring")
        assert "agatston" in terms or "cac" in terms

    def test_osteoporosis_expansion(self):
        """Osteoporosis query expands to bone density terms."""
        terms = expand_query("osteoporosis screening ct")
        assert "bone density" in terms or "dexa" in terms

    def test_degenerative_disc_expansion(self):
        """Degenerative disc query expands to Pfirrmann terms."""
        terms = expand_query("degenerative disc disease mri")
        assert "pfirrmann" in terms or "disc herniation" in terms

    def test_shoulder_body_region_expansion(self):
        """Shoulder region expands to joint/rotator cuff terms."""
        terms = expand_query("shoulder mri")
        assert "rotator cuff" in terms or "glenohumeral" in terms

    def test_musculoskeletal_body_region_expansion(self):
        """MSK region expands to orthopedic terms."""
        terms = expand_query("musculoskeletal imaging")
        assert "msk" in terms or "orthopedic" in terms

    def test_bosniak_guideline_expansion(self):
        """Bosniak guideline expands to renal cyst terms."""
        terms = expand_query("bosniak classification")
        assert "renal cyst classification" in terms or "cystic renal mass" in terms

    def test_pfirrmann_guideline_expansion(self):
        """Pfirrmann guideline expands to disc grading terms."""
        terms = expand_query("pfirrmann grading")
        assert "disc grading" in terms or "disc degeneration grade" in terms
