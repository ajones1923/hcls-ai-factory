...
class TestSearchPlanBodyRegions:
    ...
    def test_identifies_shoulder(self, agent):
        """Search plan identifies shoulder body region."""
        plan = agent.search_plan("MRI shoulder rotator cuff tear")
        assert "shoulder" in [r.lower() for r in plan.body_regions] or "extremity" in [r.lower() for r in plan.body_regions]

    def test_identifies_pelvis_from_prostate(self, agent):
        """Search plan identifies pelvis from prostate query."""
        plan = agent.search_plan("MRI prostate PI-RADS assessment")
        assert any("pelvis" in r.lower() or "prostate" in r.lower() for r in plan.body_regions)

    def test_identifies_neck_from_thyroid(self, agent):
        """Search plan identifies neck from thyroid query."""
        plan = agent.search_plan("ultrasound thyroid nodule evaluation")
        assert "neck" in plan.body_regions

    def test_identifies_abdomen_from_appendicitis(self, agent):
        """Search plan identifies abdomen from appendicitis query."""
        plan = agent.search_plan("CT abdomen appendicitis evaluation")
        assert "abdomen" in plan.body_regions

    def test_identifies_musculoskeletal(self, agent):
        """Search plan identifies musculoskeletal body region."""
        plan = agent.search_plan("musculoskeletal MRI bone tumor")
        assert "musculoskeletal" in plan.body_regions
...
# ===================================================================
# Comparative evidence combination (bug fix verification)
# ===================================================================


class TestComparativeEvidence:
    """Verify comparative search combines evidence from both sides."""

    @pytest.fixture
    def agent_with_comparative_rag(self, mock_collection_manager, mock_embedder, mock_llm_client):
        """Agent with RAG engine that returns comparative results."""
        engine = ImagingRAGEngine(mock_collection_manager, mock_embedder, mock_llm_client)

        # Set up retrieve_comparative to return both sides
        evidence_a = CrossCollectionResult(
            query="CT hemorrhage",
            hits=[
                SearchHit(collection="imaging_literature", id="a1", score=0.9, text="CT evidence for hemorrhage detection"),
                SearchHit(collection="imaging_literature", id="a2", score=0.8, text="CT sensitivity for ICH"),
            ],
            total_collections_searched=5,
            search_time_ms=20.0,
        )
        evidence_b = CrossCollectionResult(
            query="MRI hemorrhage",
            hits=[
                SearchHit(collection="imaging_literature", id="b1", score=0.85, text="MRI evidence for hemorrhage detection"),
                SearchHit(collection="imaging_literature", id="b2", score=0.75, text="MRI SWI for microbleeds"),
            ],
            total_collections_searched=5,
            search_time_ms=25.0,
        )
        ...
        comp = MagicMock()
        comp.evidence_a = evidence_a
        comp.evidence_b = evidence_b
        comp.entity_a = "ct"
        comp.entity_b = "mri"
        comp.entity_a_resolved = {"full_name": "Computed Tomography", "aliases": ["CT"]}
        comp.entity_b_resolved = {"full_name": "Magnetic Resonance Imaging", "aliases": ["MRI"]}
        comp.shared_evidence = []
        comp.comparison_context = "CT vs MRI comparison for hemorrhage"
        comp.total_search_time_ms = 45.0
        engine.retrieve_comparative = MagicMock(return_value=comp)

        return ImagingIntelligenceAgent(rag_engine=engine)

    def test_combined_evidence_includes_both_sides(self, agent_with_comparative_rag):
        """Comparative search result includes hits from both evidence_a and evidence_b."""
        query = AgentQuery(question="CT vs MRI for hemorrhage detection")
        response = agent_with_comparative_rag.run(query)
        hit_ids = [h.id for h in response.evidence.hits]
        assert "a1" in hit_ids, "Missing hit from evidence_a"
        assert "b1" in hit_ids, "Missing hit from evidence_b"

    def test_combined_evidence_total_hit_count(self, agent_with_comparative_rag):
        """Combined evidence should contain all hits from both sides."""
        query = AgentQuery(question="CT vs MRI for hemorrhage detection")
        response = agent_with_comparative_rag.run(query)
        assert response.evidence.hit_count == 4

    def test_combined_evidence_sorted_by_score(self, agent_with_comparative_rag):
        """Combined hits should be sorted by score in descending order."""
        query = AgentQuery(question="CT vs MRI for hemorrhage detection")
        response = agent_with_comparative_rag.run(query)
        scores = [h.score for h in response.evidence.hits]
        assert scores == sorted(scores, reverse=True)

    def test_combined_evidence_preserves_comparison_context(self, agent_with_comparative_rag):
        """Knowledge context should carry over from comparison_context."""
        query = AgentQuery(question="CT vs MRI for hemorrhage detection")
        response = agent_with_comparative_rag.run(query)
        assert "CT vs MRI" in response.evidence.knowledge_context

    def test_combined_evidence_sums_collections_searched(self, agent_with_comparative_rag):
        """Total collections searched should be sum of both sides."""
        query = AgentQuery(question="CT vs MRI for hemorrhage detection")
        response = agent_with_comparative_rag.run(query)
        assert response.evidence.total_collections_searched == 10
