...
class TestImagingPathologies:
    ...
    def test_emphysema_exists(self):
        assert "emphysema" in IMAGING_PATHOLOGIES

    def test_lymphadenopathy_exists(self):
        assert "lymphadenopathy" in IMAGING_PATHOLOGIES

    def test_breast_mass_exists(self):
        assert "breast_mass" in IMAGING_PATHOLOGIES

    def test_thyroid_nodule_exists(self):
        assert "thyroid_nodule" in IMAGING_PATHOLOGIES

    def test_pancreatic_mass_exists(self):
        assert "pancreatic_mass" in IMAGING_PATHOLOGIES

    def test_bowel_obstruction_exists(self):
        assert "bowel_obstruction" in IMAGING_PATHOLOGIES

    def test_appendicitis_exists(self):
        assert "appendicitis" in IMAGING_PATHOLOGIES

    def test_coronary_calcification_exists(self):
        assert "coronary_calcification" in IMAGING_PATHOLOGIES

    def test_osteoporosis_exists(self):
        assert "osteoporosis" in IMAGING_PATHOLOGIES

    def test_degenerative_disc_exists(self):
        assert "degenerative_disc" in IMAGING_PATHOLOGIES
...
class TestImagingModalities:
    ...
    def test_pet_standalone_exists(self):
        assert "pet" in IMAGING_MODALITIES

    def test_has_9_modalities(self):
        assert len(IMAGING_MODALITIES) == 9
...
class TestImagingAnatomy:
    ...
    def test_prostate_exists(self):
        assert "prostate" in IMAGING_ANATOMY

    def test_adrenal_glands_exists(self):
        assert "adrenal_glands" in IMAGING_ANATOMY

    def test_esophagus_exists(self):
        assert "esophagus" in IMAGING_ANATOMY

    def test_stomach_exists(self):
        assert "stomach" in IMAGING_ANATOMY

    def test_uterus_exists(self):
        assert "uterus" in IMAGING_ANATOMY

    def test_shoulder_exists(self):
        assert "shoulder" in IMAGING_ANATOMY
...
class TestGetKnowledgeStats:
    ...
    def test_modality_count_is_9(self):
        stats = get_knowledge_stats()
        assert stats["modalities"] == 9
    ...
    def test_pathology_relationships_count(self):
        stats = get_knowledge_stats()
        assert stats["pathology_relationships"] >= 12
...
class TestPathologyRelationships:
    """Tests for the PATHOLOGY_RELATIONSHIPS dict and helper functions."""

    def test_pathology_relationships_exist(self):
        assert len(PATHOLOGY_RELATIONSHIPS) > 0
        assert len(PATHOLOGY_RELATIONSHIPS) >= 12

    def test_pe_causes_rv_strain(self):
        pe = PATHOLOGY_RELATIONSHIPS["pulmonary_embolism"]
        assert "right_ventricular_strain" in pe["can_cause"]

    def test_lung_nodule_can_indicate_cancer(self):
        ln = PATHOLOGY_RELATIONSHIPS["lung_nodule"]
        assert "primary_lung_cancer" in ln["can_indicate"]

    def test_lung_nodule_can_indicate_granuloma(self):
        ln = PATHOLOGY_RELATIONSHIPS["lung_nodule"]
        assert "granuloma" in ln["can_indicate"]

    def test_ich_risk_factors_include_hypertension(self):
        ich = PATHOLOGY_RELATIONSHIPS["intracranial_hemorrhage"]
        assert "hypertension" in ich["risk_factors"]

    def test_pneumonia_differential(self):
        pna = PATHOLOGY_RELATIONSHIPS["pneumonia"]
        assert "pulmonary_edema" in pna["differential"]

    def test_aortic_dissection_differential(self):
        ad = PATHOLOGY_RELATIONSHIPS["aortic_dissection"]
        assert "pulmonary_embolism" in ad["differential"]

    def test_breast_cancer_metastasis(self):
        bc = PATHOLOGY_RELATIONSHIPS["breast_cancer"]
        assert "bone_metastasis" in bc["can_cause"]

    def test_get_related_pathologies(self):
        result = get_related_pathologies("pulmonary_embolism")
        assert isinstance(result, dict)
        assert "can_cause" in result
        assert "risk_factors" in result

    def test_get_related_pathologies_case_insensitive(self):
        result = get_related_pathologies("PULMONARY_EMBOLISM")
        assert "can_cause" in result

    def test_get_related_pathologies_spaces(self):
        result = get_related_pathologies("pulmonary embolism")
        assert "can_cause" in result

    def test_get_differential_diagnosis(self):
        diff = get_differential_diagnosis("pneumonia")
        assert isinstance(diff, list)
        assert len(diff) > 0
        assert "pulmonary_edema" in diff

    def test_get_differential_diagnosis_no_differential(self):
        diff = get_differential_diagnosis("lung_nodule")
        assert diff == []

    def test_get_risk_factors(self):
        rf = get_risk_factors("lung_nodule")
        assert isinstance(rf, list)
        assert len(rf) > 0
        assert "smoking" in rf

    def test_get_risk_factors_coronary(self):
        rf = get_risk_factors("coronary_artery_disease")
        assert "hypertension" in rf
        assert "diabetes" in rf

    def test_unknown_pathology_returns_empty(self):
        result = get_related_pathologies("nonexistent_pathology_xyz")
        assert result == {}

    def test_unknown_differential_returns_empty(self):
        diff = get_differential_diagnosis("nonexistent_pathology_xyz")
        assert diff == []

    def test_unknown_risk_factors_returns_empty(self):
        rf = get_risk_factors("nonexistent_pathology_xyz")
        assert rf == []
