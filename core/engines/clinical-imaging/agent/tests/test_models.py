...
class TestBodyRegion:
    ...
    def test_has_shoulder(self):
        assert BodyRegion.SHOULDER.value == "shoulder"

    def test_has_musculoskeletal(self):
        assert BodyRegion.MUSCULOSKELETAL.value == "musculoskeletal"

    def test_count(self):
        assert len(BodyRegion) == 16
...
class TestFindingCategory:
    ...
    def test_has_obstruction(self):
        assert FindingCategory.OBSTRUCTION.value == "obstruction"

    def test_has_inflammation(self):
        assert FindingCategory.INFLAMMATION.value == "inflammation"

    def test_has_degeneration(self):
        assert FindingCategory.DEGENERATION.value == "degeneration"

    def test_has_fibrosis(self):
        assert FindingCategory.FIBROSIS.value == "fibrosis"

    def test_has_emphysema_finding(self):
        assert FindingCategory.EMPHYSEMA_FINDING.value == "emphysema"

    def test_has_lymphadenopathy(self):
        assert FindingCategory.LYMPHADENOPATHY.value == "lymphadenopathy"

    def test_has_thyroid_nodule(self):
        assert FindingCategory.THYROID_NODULE.value == "thyroid_nodule"

    def test_count(self):
        assert len(FindingCategory) == 37
...
class TestTIRADS:
    """Verify TIRADS enum members."""

    def test_tr1(self):
        assert TIRADS.TR1.value == "TR1"

    def test_tr5(self):
        assert TIRADS.TR5.value == "TR5"

    def test_count(self):
        assert len(TIRADS) == 5

    def test_expected_members(self):
        names = {m.name for m in TIRADS}
        assert names == {"TR1", "TR2", "TR3", "TR4", "TR5"}


class TestLIRADS:
    """Verify LIRADS enum members."""

    def test_lr_1(self):
        assert LIRADS.LR_1.value == "LR-1"

    def test_lr_5(self):
        assert LIRADS.LR_5.value == "LR-5"

    def test_lr_m(self):
        assert LIRADS.LR_M.value == "LR-M"

    def test_lr_tiv(self):
        assert LIRADS.LR_TIV.value == "LR-TIV"

    def test_count(self):
        assert len(LIRADS) == 7

    def test_expected_members(self):
        names = {m.name for m in LIRADS}
        assert names == {"LR_1", "LR_2", "LR_3", "LR_4", "LR_5", "LR_M", "LR_TIV"}
