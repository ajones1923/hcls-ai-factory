...
# ===================================================================
# BREAST BI-RADS TRIGGERS
# ===================================================================


def _breast_result(classification: str, severity: FindingSeverity) -> WorkflowResult:
    """Helper to build a breast_birads WorkflowResult."""
    return WorkflowResult(
        ...
        findings=[{
            "category": "mass",
            "description": f"Breast mass, {classification}",
            "severity": severity.value,
        }],
        is_mock=True,
    )
...
class TestBreastBiradsTrigger:
    """Tests for BI-RADS cross-modal triggers."""

    def test_birads_4c_triggers(self, cross_modal_trigger):
        result = _breast_result("BI-RADS 4C", FindingSeverity.URGENT)
        cm = cross_modal_trigger.evaluate(result)
        assert cm is not None
        assert isinstance(cm, CrossModalResult)
        assert "4C" in cm.trigger_reason
        assert cm.query_count == len(BREAST_GENOMICS_QUERIES)

    def test_birads_5_triggers(self, cross_modal_trigger):
        result = _breast_result("BI-RADS 5", FindingSeverity.CRITICAL)
        cm = cross_modal_trigger.evaluate(result)
        assert cm is not None
        assert "5" in cm.trigger_reason

    def test_birads_4a_triggers(self, cross_modal_trigger):
        result = _breast_result("BI-RADS 4A", FindingSeverity.URGENT)
        cm = cross_modal_trigger.evaluate(result)
        assert cm is not None
        assert "4A" in cm.trigger_reason

    def test_birads_3_does_not_trigger(self, cross_modal_trigger):
        result = _breast_result("BI-RADS 3", FindingSeverity.SIGNIFICANT)
        cm = cross_modal_trigger.evaluate(result)
        assert cm is None

    def test_birads_2_does_not_trigger(self, cross_modal_trigger):
        result = _breast_result("BI-RADS 2", FindingSeverity.ROUTINE)
        cm = cross_modal_trigger.evaluate(result)
        assert cm is None

    def test_birads_1_does_not_trigger(self, cross_modal_trigger):
        result = _breast_result("BI-RADS 1", FindingSeverity.NORMAL)
        cm = cross_modal_trigger.evaluate(result)
        assert cm is None


# ===================================================================
# THYROID TI-RADS TRIGGERS
# ===================================================================


def _thyroid_result(classification: str, severity: FindingSeverity) -> WorkflowResult:
    """Helper to build a thyroid_tirads WorkflowResult."""
    return WorkflowResult(
        workflow_name="thyroid_tirads",
        status=WorkflowStatus.COMPLETED,
        classification=classification,
        severity=severity,
        findings=[{
            "category": "nodule",
            "description": f"Thyroid nodule, {classification}",
            "severity": severity.value,
        }],
        is_mock=True,
    )
...
class TestThyroidTiradsTrigger:
    """Tests for TI-RADS cross-modal triggers."""

    def test_tirads_tr5_triggers(self, cross_modal_trigger):
        result = _thyroid_result("TI-RADS TR5", FindingSeverity.CRITICAL)
        cm = cross_modal_trigger.evaluate(result)
        assert cm is not None
        assert isinstance(cm, CrossModalResult)
        assert "TR5" in cm.trigger_reason
        assert cm.query_count == len(THYROID_GENOMICS_QUERIES)

    def test_tirads_tr4_triggers(self, cross_modal_trigger):
        result = _thyroid_result("TI-RADS TR4", FindingSeverity.URGENT)
        cm = cross_modal_trigger.evaluate(result)
        assert cm is not None
        assert "TR4" in cm.trigger_reason

    def test_tirads_tr3_does_not_trigger(self, cross_modal_trigger):
        result = _thyroid_result("TI-RADS TR3", FindingSeverity.SIGNIFICANT)
        cm = cross_modal_trigger.evaluate(result)
        assert cm is None

    def test_tirads_tr2_does_not_trigger(self, cross_modal_trigger):
        result = _thyroid_result("TI-RADS TR2", FindingSeverity.ROUTINE)
        cm = cross_modal_trigger.evaluate(result)
        assert cm is None

    def test_tirads_tr1_does_not_trigger(self, cross_modal_trigger):
        result = _thyroid_result("TI-RADS TR1", FindingSeverity.NORMAL)
        cm = cross_modal_trigger.evaluate(result)
        assert cm is None


# ===================================================================
# LIVER LI-RADS TRIGGERS
# ===================================================================


def _liver_result(classification: str, severity: FindingSeverity) -> WorkflowResult:
    """Helper to build a liver_lirads WorkflowResult."""
    return WorkflowResult(
        workflow_name="liver_lirads",
        status=WorkflowStatus.COMPLETED,
        classification=classification,
        severity=severity,
        findings=[{
            "category": "observation",
            "description": f"Liver observation, {classification}",
            "severity": severity.value,
        }],
        is_mock=True,
    )
...
class TestLiverLiradsTrigger:
    """Tests for LI-RADS cross-modal triggers."""

    def test_lirads_lr5_triggers(self, cross_modal_trigger):
        result = _liver_result("LI-RADS LR-5", FindingSeverity.CRITICAL)
        cm = cross_modal_trigger.evaluate(result)
        assert cm is not None
        assert isinstance(cm, CrossModalResult)
        assert "LR-5" in cm.trigger_reason
        assert cm.query_count == len(LIVER_GENOMICS_QUERIES)

    def test_lirads_lr4_triggers(self, cross_modal_trigger):
        result = _liver_result("LI-RADS LR-4", FindingSeverity.URGENT)
        cm = cross_modal_trigger.evaluate(result)
        assert cm is not None
        assert "LR-4" in cm.trigger_reason

    def test_lirads_lr3_does_not_trigger(self, cross_modal_trigger):
        result = _liver_result("LI-RADS LR-3", FindingSeverity.SIGNIFICANT)
        cm = cross_modal_trigger.evaluate(result)
        assert cm is None

    def test_lirads_lr2_does_not_trigger(self, cross_modal_trigger):
        result = _liver_result("LI-RADS LR-2", FindingSeverity.ROUTINE)
        cm = cross_modal_trigger.evaluate(result)
        assert cm is None

    def test_lirads_lr1_does_not_trigger(self, cross_modal_trigger):
        result = _liver_result("LI-RADS LR-1", FindingSeverity.NORMAL)
        cm = cross_modal_trigger.evaluate(result)
        assert cm is None
