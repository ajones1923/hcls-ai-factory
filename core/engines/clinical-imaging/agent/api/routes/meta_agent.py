...
@router.post("/v1/imaging/integrated-assessment")
async def integrated_assessment(request: dict):
    ...
    Queries oncology, clinical trial, cardiology, and neurology agents
    for a comprehensive coordinated imaging assessment.
    """
    try:
        from agent.src.cross_agent import (
        ...
        imaging_context = request.get("imaging_context", {})
        neurological_context = request.get("neurological_context", {})

        results = []

        # Query oncology agent for staging requirements
        if cancer_type:
            results.append(query_oncology_agent(cancer_type, patient_context=patient_profile))

        # Query trial agent for imaging requirements
        if patient_profile:
            results.append(query_trial_agent(patient_profile))

        # Query cardiology agent for cardiac imaging coordination
        if patient_id:
            results.append(query_cardiology_agent(patient_id, imaging_context=imaging_context))

        # Query neurology agent for CNS imaging coordination
        if patient_id:
            results.append(query_neurology_agent(patient_id, neurological_context=neurological_context))
