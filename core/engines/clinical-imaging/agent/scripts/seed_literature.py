#!/usr/bin/env python3
"""Seed the imaging_literature Milvus collection from reference JSON data.

Loads literature_seed_data.json, validates each record as an ImagingLiterature
Pydantic model, generates BGE-small-en-v1.5 embeddings, and inserts
into the imaging_literature collection.

Usage:
    python scripts/seed_literature.py
    python scripts/seed_literature.py --host 10.0.0.1 --port 19530
...
SEED_FILE = "literature_seed_data.json"


def main():
    parser = argparse.ArgumentParser(
        description="Seed imaging_literature collection from reference data"
    )
    ...
    print("=" * 65)
    print("  Imaging Intelligence Agent — Seed Literature")
    print("=" * 65)
    ...
    for item in raw_data:
        try:
            record = ImagingLiterature(**item)
            records.append(record)
        except Exception as e:
            logger.warning(f"Skipping invalid record: {e}")

    logger.info(f"Validated {len(records)} ImagingLiterature records")
    ...
    print()
    print("=" * 65)
    print(f"  Seeded {count} literature records into {COLLECTION_NAME}")
    print("=" * 65)
