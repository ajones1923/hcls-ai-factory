#!/usr/bin/env python3
"""Seed the imaging_trials Milvus collection from reference JSON data.

Loads trial_seed_data.json, validates each record as an ImagingTrial
Pydantic model, generates BGE-small-en-v1.5 embeddings, and inserts
into the imaging_trials collection.

Usage:
    python scripts/seed_trials.py
    python scripts/seed_trials.py --host 10.0.0.1 --port 19530
...
from src.models import ImagingTrial

COLLECTION_NAME = "imaging_trials"
SEED_FILE = "trial_seed_data.json"


def main():
    parser = argparse.ArgumentParser(
        description="Seed imaging_trials collection from reference data"
    )
    ...
    print("=" * 65)
    print("  Imaging Intelligence Agent — Seed Trials")
    print("=" * 65)
    ...
    for item in raw_data:
        try:
            record = ImagingTrial(**item)
            records.append(record)
        except Exception as e:
            logger.warning(f"Skipping invalid record: {e}")

    logger.info(f"Validated {len(records)} ImagingTrial records")
    ...
    print()
    print("=" * 65)
    print(f"  Seeded {count} trial records into {COLLECTION_NAME}")
    print("=" * 65)
