...
PEDIATRIC ONCOLOGY & PHARMACOLOGY GUIDANCE:
When asked about pediatric patients, childhood cancers, or pediatric drug considerations:

Dosing Principles:
- Pediatric dosing must be BSA-scaled (mg/m²), NOT fixed adult doses
- Body surface area (BSA) changes rapidly in growing children — recalculate at each cycle
- Neonatal and infant dosing may require further reductions beyond BSA scaling
- Weight-based dosing (mg/kg) is used for some agents but BSA-based is standard for chemotherapy

Cumulative Lifetime Toxicity Limits:
- Anthracyclines (doxorubicin, daunorubicin): 450 mg/m² cumulative limit (cardiotoxicity)
  * Pediatric patients have decades of life ahead — cardiac monitoring is critical
  * Dexrazoxane may be used as cardioprotectant in high cumulative dose settings
- Bleomycin: cumulative pulmonary toxicity, lifetime lung function monitoring required
- Radiation: cumulative dose limits by organ; growth plate damage in children

Developing Brain Considerations:
- Methotrexate: CNS toxicity risk especially with intrathecal administration
  * Leukoencephalopathy, neurocognitive decline in developing brains
- Vincristine: peripheral neuropathy — dose cap at 2mg absolute (not per m²)
  * Neuropathy may be more impactful in developing motor systems
- Cranial radiation: neurocognitive late effects (IQ decline, learning disabilities)
  * Avoided when possible in children <3 years; proton therapy preferred
- Cisplatin/carboplatin: ototoxicity — hearing loss affects speech development in young children

Age-Stratified Recommendations:
- Neonatal (0-28 days): extremely limited drug metabolism, immature renal/hepatic function
- Infant (1-23 months): rapidly maturing organ function, CYP3A4 reaches adult levels by age 1-2
- Child (2-12 years): often higher mg/kg clearance than adults for many drugs
- Adolescent (13-17 years): approaching adult pharmacokinetics but still developing
  * Fertility preservation must be discussed before gonadotoxic therapy
  * Psychosocial considerations for treatment adherence

Common Pediatric Cancer Types:
- ALL (Acute Lymphoblastic Leukemia): most common childhood cancer (~25% of pediatric cancers)
  * Key genes: BCR-ABL1, ETV6-RUNX1, MLL rearrangements, IKZF1, PAX5
- Neuroblastoma: MYCN amplification, ALK mutations, segmental chromosomal aberrations
- Medulloblastoma: WNT, SHH, Group 3, Group 4 molecular subtypes
- Wilms tumor (nephroblastoma): WT1, CTNNB1, AMER1 mutations
- Rhabdomyosarcoma: PAX3-FOXO1, PAX7-FOXO1 fusions (alveolar subtype)
- Ewing sarcoma: EWSR1-FLI1 fusion (pathognomonic)
- Osteosarcoma: TP53, RB1 mutations; complex karyotypes

Pharmacogenomics in Pediatrics:
- TPMT/NUDT15: Critical for thiopurine dosing (6-mercaptopurine in ALL maintenance)
  * Poor metabolizers require 10-fold dose reduction to avoid fatal myelosuppression
  * NUDT15 variants especially prevalent in East Asian populations
- CYP3A5: Affects vincristine metabolism — expressors may have lower exposure
- DPYD: Dihydropyrimidine dehydrogenase deficiency — 5-FU/capecitabine toxicity
- UGT1A1: Irinotecan metabolism — Gilbert syndrome increases toxicity risk
- Report ANY variants in TPMT, NUDT15, CYP3A5, DPYD, UGT1A1 for pediatric patients
...
# Pediatric Oncology query expansion
PEDIATRIC_GENES = {
    # Pediatric cancers
    'pediatric': ['TPMT', 'NUDT15', 'CYP3A5', 'DPYD', 'UGT1A1', 'MYCN', 'ALK', 'WT1', 'EWSR1', 'PAX3', 'RB1'],
    'childhood cancer': ['TPMT', 'NUDT15', 'CYP3A5', 'MYCN', 'ALK', 'WT1', 'EWSR1', 'PAX3'],
    'all leukemia': ['BCR-ABL1', 'ETV6', 'RUNX1', 'IKZF1', 'PAX5', 'MLL', 'TPMT', 'NUDT15'],
    'acute lymphoblastic': ['BCR-ABL1', 'ETV6', 'RUNX1', 'IKZF1', 'PAX5', 'MLL', 'TPMT', 'NUDT15'],
    'neuroblastoma': ['MYCN', 'ALK', 'PHOX2B', 'TERT'],
    'medulloblastoma': ['PTCH1', 'SUFU', 'SMO', 'CTNNB1', 'TP53', 'MYC'],
    'wilms': ['WT1', 'CTNNB1', 'AMER1', 'TP53'],
    'nephroblastoma': ['WT1', 'CTNNB1', 'AMER1', 'TP53'],
    'rhabdomyosarcoma': ['PAX3', 'PAX7', 'FOXO1', 'MYOD1'],
    'ewing sarcoma': ['EWSR1', 'FLI1', 'ERG'],
    'ewing': ['EWSR1', 'FLI1', 'ERG'],
    'osteosarcoma': ['TP53', 'RB1', 'MDM2'],
    'retinoblastoma': ['RB1'],
    # Pediatric pharmacogenomics
    'thiopurine': ['TPMT', 'NUDT15'],
    'mercaptopurine': ['TPMT', 'NUDT15'],
    '6-mp': ['TPMT', 'NUDT15'],
    'vincristine': ['CYP3A5', 'CYP3A4'],
    'methotrexate': ['MTHFR', 'SLC19A1', 'ABCB1', 'ABCC2'],
    # Pediatric dosing genes
    'anthracycline': ['RARG', 'SLC28A3', 'UGT1A6', 'ABCC1'],
    'doxorubicin': ['RARG', 'SLC28A3', 'UGT1A6', 'ABCC1'],
    'cardiotoxicity': ['RARG', 'SLC28A3', 'HAS3'],
}
...
def _verify_answer(answer: str, evidence, llm=None) -> Dict[str, Any]:
    """ORCH-11: run the shared HCLS verify-gate on a generated answer. The deterministic
    clinical-honesty register always runs (no LLM cost); pass `llm` to also run adversarial
    claim-checking. Path-robust + fail-safe: never breaks the RAG path if the lib is absent."""
    try:
        import sys
        from pathlib import Path
        lib = str(Path(__file__).resolve().parents[2] / "lib")  # hcls-ai-factory/lib
        if lib not in sys.path:
            sys.path.insert(0, lib)
        from hcls_common.verify_gate import verify_text
        return verify_text(answer, evidence=evidence, llm=llm)
    except Exception as e:  # gate must never break generation
        logger.warning(f"verify-gate unavailable ({type(e).__name__}); skipping")
        return {"status": "unverified", "honesty_violations": [], "flagged": [],
                "summary": {"llm_checked": False}}
...
class RAGEngine:
    ...
    def query(
        ...
        Args:
            ...
            adversarial_verify: also run the LLM adversarial claim-check (extra LLM calls);
                off by default — the deterministic honesty register always runs regardless.

        Returns:
            Dict with 'answer', 'evidence', 'query', and 'verify' keys
        """
        ...
        # ORCH-11 verify-gate: deterministic honesty register always runs (no LLM cost);
        # adversarial claim-check only when explicitly requested.
        verdict = _verify_answer(answer, evidence,
                                 llm=self.llm if adversarial_verify else None)
        if any(v.get("severity") == "block" for v in verdict.get("honesty_violations", [])):
            # Never publish a bare overclaim — append an explicit research-use correction.
            answer += ("\n\n> ⚠ Automated honesty check flagged an unverified overclaim in "
                       "this response. Treat it as research-use decision support only — not a "
                       "clinical diagnosis, treatment directive, or regulatory-cleared result; "
                       "clinician review required.")
            verdict["disclaimer_appended"] = True
