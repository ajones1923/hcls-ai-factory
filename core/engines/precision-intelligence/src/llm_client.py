...
def _record_usage(model, usage) -> None:
    """COST-2: record an Anthropic call's token usage to the HCLS ledger. Best-effort —
    a ledger failure (import or write) must never disrupt generation."""
    if usage is None:
        return
    try:
        try:
            ...
            import token_ledger  # when run as a top-level module (src on sys.path)
        token_ledger.record(model, usage)
    except Exception:
        pass
...
class AnthropicClient(BaseLLMClient):
    ...
    def generate(
        ...
        message = self.client.messages.create(
            ...
            # Prompt caching (COST-12): cache the large, reused system prompt (knowledge +
            # messaging context). GA, no beta header; silently no-ops below the per-model
            # minimum. Cuts cached-input cost ~90% across repeated queries.
    ...
    def generate_stream(
        ...
        with self.client.messages.stream(
            ...
            max_tokens=max_tokens,
            temperature=temperature,
            system=([{"type": "text", "text": system_prompt,            # COST-12 prompt caching
                      "cache_control": {"type": "ephemeral"}}] if system_prompt else ""),
            messages=[
                {"role": "user", "content": prompt}
            ]
            ...
            for text in stream.text_stream:
                yield text
            try:                                                      # COST-2 ledger
                _record_usage(self.model, getattr(stream.get_final_message(), "usage", None))
            except Exception:
                pass
