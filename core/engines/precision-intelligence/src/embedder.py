    ...
    def _load_cache(self) -> None:
        ...
        if cache_file.exists():
            try:
                ...
                # Cache stores keys as 'key_<variant_id>' and values as numpy arrays
                self._memory_cache = {
                    k[len("key_"):]: data[k]
                    for k in data.files
                ...
                logger.warning(f"Could not load cache (will regenerate): {e}")
                self._memory_cache = {}

    def _save_cache(self) -> None:
        """Save cached embeddings to disk."""
        try:
            # Store each embedding as a separate array keyed by 'key_<variant_id>'
            # This avoids pickle serialization entirely
            arrays = {f"key_{k}": v for k, v in self._memory_cache.items()}
