...
def get_variant_stats() -> Dict[str, int]:
    ...
    try:
        ...
        # NOTE: Do NOT disconnect "default" here — the RAGEngine shares
        # this process-global alias.  Disconnecting it kills search for
        # every subsequent user query until the next reconnect.
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        <div style="display: flex; align-items: center; gap: 0.5rem; margin-top: -0.5rem;">
            ...
            <span style="color: #8b949e; font-size: 0.8rem;">{html.escape(model_info['description'])} · {html.escape(model_info['size'])}</span>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div style="text-align: center; padding: 0.5rem; background: #ffffff; border-radius: 8px; border: 1px solid #e1e4e8;">
            ...
            <span style="color: #6b7280; font-size: 0.75rem;"> {html.escape(model_name)} Active</span>
        </div>
        """, unsafe_allow_html=True)
...
def render_file_manager():
    ...
    # ═══════════════════════════════════════════════════════════
    st.markdown("---")
    st.markdown(f'<p style="color: #2ea44f; font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 1px;">Files in {html.escape(selected_dir.upper())}</p>', unsafe_allow_html=True)
...
def _sanitize_filter_value(value: str) -> str:
    """Sanitize a user-provided filter value for safe use in Milvus expressions.

    Strips characters that could inject into Milvus boolean expressions:
    double quotes, backslashes, parentheses, semicolons, and boolean
    operators (and/or/not/in/like). Only allows alphanumeric characters,
    ...
    Returns:
        Sanitized string safe for interpolation into Milvus filter expressions.
    """
    import re
    # Remove any characters that are not alphanumeric, hyphens, underscores,
    # dots, colons, or spaces. This blocks quotes, backslashes, parens,
    # and semicolons which could alter the filter expression semantics.
    sanitized = re.sub(r'[^a-zA-Z0-9\-_.\s:]', '', value)
    # Collapse whitespace and strip
    sanitized = re.sub(r'\s+', ' ', sanitized).strip()
    # Cap length to prevent overly long filter values
...
def get_filter_expression():
    ...
    All user-provided text inputs are sanitized before interpolation
    into Milvus boolean filter expressions to prevent injection attacks.
    """
    filter_parts = []
    ...
    if filter_gene:
        safe_gene = _sanitize_filter_value(filter_gene)
        if safe_gene:
            filter_parts.append(f'gene == "{safe_gene}"')
    if filter_chrom != "All":
        safe_chrom = _sanitize_filter_value(filter_chrom)
        if safe_chrom:
            filter_parts.append(f'chrom == "{safe_chrom}"')
    if filter_impact:
        # filter_impact comes from a multiselect with fixed options,
        # but sanitize defensively in case the widget is bypassed
        safe_impacts = [_sanitize_filter_value(i) for i in filter_impact if _sanitize_filter_value(i)]
        if safe_impacts:
            impact_expr = " or ".join([f'impact == "{i}"' for i in safe_impacts])
        ...
        for conn in genes_with_connections[:3]:  # Limit to top 3 for clarity
            ...
            with cols[0]:
                ...
                <div style="background: linear-gradient(135deg, #fef3c7 0%, #fde68a 100%); border: 2px solid #f59e0b; border-radius: 8px; padding: 8px; text-align: center;">
                    ...
                    <div style="font-size: 0.8rem; font-weight: 600; color: #78350f;">{html.escape(conn['variant'][:12])}</div>
                    {am_line}
                </div>
                """, unsafe_allow_html=True)
            ...
            with cols[1]:
                ...
                <div style="background: linear-gradient(135deg, #dbeafe 0%, #bfdbfe 100%); border: 2px solid #3b82f6; border-radius: 8px; padding: 8px; text-align: center;">
                    ...
                    <div style="font-size: 0.8rem; font-weight: 600; color: #1e3a8a;">{html.escape(conn['gene'])}</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with cols[2]:
                ...
                <div style="background: linear-gradient(135deg, #d1fae5 0%, #a7f3d0 100%); border: 2px solid #10b981; border-radius: 8px; padding: 8px; text-align: center;">
                    ...
                    <div style="font-size: 0.8rem; font-weight: 600; color: #064e3b;">{html.escape(conn['protein'][:12])}</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with cols[3]:
                ...
                <div style="background: linear-gradient(135deg, #e0e7ff 0%, #c7d2fe 100%); border: 2px solid #6366f1; border-radius: 8px; padding: 8px; text-align: center;">
                    ...
                    <div style="font-size: 0.8rem; font-weight: 600; color: #312e81;">{html.escape(conn['pathway'][:12])}</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with cols[4]:
                ...
                <div style="background: linear-gradient(135deg, #fee2e2 0%, #fecaca 100%); border: 2px solid #ef4444; border-radius: 8px; padding: 8px; text-align: center;">
                    ...
                    <div style="font-size: 0.8rem; font-weight: 600; color: #7f1d1d;">{html.escape(primary_disease.split('(')[0].strip()[:12])}</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with cols[5]:
                ...
                <div style="background: linear-gradient(135deg, #76b900 0%, #5a8c00 100%); border: 2px solid #4a7c00; border-radius: 8px; padding: 8px; text-align: center;">
                    ...
                    <div style="font-size: 0.8rem; font-weight: 600; color: white;">{html.escape(primary_drug.split('(')[0].strip()[:12])}</div>
                </div>
                """, unsafe_allow_html=True)
...
def trigger_report_refresh(query: str, evidence: list, response: str):
    ...
    def _regenerate():
        try:
            ...
            generator_script = base_dir / 'core/engines/therapeutic-discovery' / 'generate_vcp_report_enhanced.py'
            generator_python = base_dir / 'core/engines/therapeutic-discovery' / 'venv' / 'bin' / 'python'
...
def main():
    ...
    <div class="stats-container">
        ...
        <div class="stat-box">
            <div class="stat-value" style="font-size: 1rem;">{html.escape(model_name)}</div>
