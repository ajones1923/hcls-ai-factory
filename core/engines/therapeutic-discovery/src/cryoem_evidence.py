...
class CryoEMEvidenceManager:
    ...
    def auto_load_structures(self, gene: str, pdb_ids: List[str] = None) -> List[CryoEMStructure]:
        """Load structures for a gene, filtering to specific PDB IDs if provided."""
        structures = self.load_structures_for_gene(gene)
        if pdb_ids and structures:
            pdb_set = {pid.upper() for pid in pdb_ids}
            filtered = [s for s in structures if s.structure_id.replace("PDB:", "").upper() in pdb_set]
            return filtered if filtered else structures
        return structures
