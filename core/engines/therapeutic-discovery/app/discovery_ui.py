...
def render_molecule_generation():
    ...
    try:
        ...
        # Simplified inline viewer for Drug Discovery
        pdb_id = st.session_state.get("target_pdb", "5FTK")
        viewer_html = f'''
        <div style="border: 1px solid #76B900; border-radius: 8px; overflow: hidden;">
            <iframe src="https://www.ebi.ac.uk/pdbe/entry/pdb/{pdb_id.lower()}"
                    width="100%" height="400" frameborder="0"
                    style="background: #0a0a0f;"></iframe>
        </div>
        '''
        # Use PDBe 3D viewer component
        molstar_html = f'''
        ...
        <div id="drugdisc-viewer" style="width:100%;height:400px;position:relative;background:#0a0a0f;border-radius:8px;overflow:hidden;"></div>
        <script>
        var viewer = new PDBeMolstarPlugin();
        viewer.render(document.getElementById('drugdisc-viewer'), {{
        ...
        '''
        components.html(molstar_html, height=420)
        st.caption(f"PDB: {pdb_id.upper()} — VCP/p97 target structure")
    except Exception:
        st.info("3D viewer requires network access to EBI PDBe")

    # -----------------------------------------------------------------
    # PGx-Aware Drug Ranking
    # -----------------------------------------------------------------
    st.markdown("---")
    st.markdown("### PGx-Personalized Ranking")
    st.caption("Drug candidates re-ranked by patient pharmacogenomic profile")

    with st.expander("Configure Patient PGx Profile", expanded=False):
        pgx_col1, pgx_col2 = st.columns(2)
        with pgx_col1:
            cyp2d6 = st.selectbox("CYP2D6", ["*1/*1 (Normal)", "*1/*2 (Normal)", "*1/*4 (Intermediate)", "*4/*4 (Poor)", "*1/*1xN (Ultra-rapid)"], key="pgx_cyp2d6")
            cyp2c19 = st.selectbox("CYP2C19", ["*1/*1 (Normal)", "*1/*2 (Intermediate)", "*2/*2 (Poor)", "*17/*17 (Ultra-rapid)"], key="pgx_cyp2c19")
        with pgx_col2:
            cyp2c9 = st.selectbox("CYP2C9", ["*1/*1 (Normal)", "*1/*2 (Intermediate)", "*2/*3 (Poor)"], key="pgx_cyp2c9")
            cyp3a5 = st.selectbox("CYP3A5", ["*1/*1 (Expresser)", "*1/*3 (Intermediate)", "*3/*3 (Poor)"], key="pgx_cyp3a5")

    # Load demo PGx profile button
    if st.button("Load Demo PGx (CYP2D6 *4/*4 Poor Metabolizer)", key="load_pgx_demo"):
        st.session_state["pgx_cyp2d6"] = "*4/*4 (Poor)"
        st.session_state["pgx_cyp2c19"] = "*1/*2 (Intermediate)"
        st.session_state["pgx_cyp2c9"] = "*1/*2 (Intermediate)"
        st.session_state["pgx_cyp3a5"] = "*3/*3 (Poor)"
        st.rerun()

    # Display PGx-ranked results if candidates exist
    if st.session_state.get("generated_molecules"):
        # Get the phenotype from selection
        def _extract_phenotype(selection: str) -> str:
            if "Poor" in selection:
                return "Poor Metabolizer"
            if "Intermediate" in selection:
                return "Intermediate Metabolizer"
            if "Ultra-rapid" in selection or "Ultra" in selection:
                return "Ultra-rapid Metabolizer"
            if "Rapid" in selection:
                return "Rapid Metabolizer"
            return "Normal Metabolizer"

        cyp2d6_phenotype = _extract_phenotype(st.session_state.get("pgx_cyp2d6", "*1/*1 (Normal)"))

        # Simple PGx compatibility scoring
        st.markdown(f"**Patient CYP2D6 Status:** {cyp2d6_phenotype}")

        if "Poor" in cyp2d6_phenotype:
            st.warning("CYP2D6 Poor Metabolizer -- Prodrugs requiring CYP2D6 activation will have reduced efficacy. Compounds primarily cleared by CYP2D6 may accumulate to toxic levels.")
            st.markdown("""
            **Recommendations for this patient:**
            - Prefer candidates metabolized via CYP3A4 or renal clearance
            - Use caution with CYP2D6 substrates (reduce dose 50%)
            - Avoid prodrugs requiring CYP2D6 bioactivation
            """)
        elif "Ultra" in cyp2d6_phenotype:
            st.warning("CYP2D6 Ultra-rapid Metabolizer -- Standard doses may be sub-therapeutic. Prodrugs may have enhanced activation.")
        else:
            st.success("CYP2D6 Normal/Intermediate -- Standard dosing appropriate for most candidates")
