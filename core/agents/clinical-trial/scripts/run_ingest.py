...
# One shared implementation: the schema-projection rules were each learned from a
# failed insert, and a per-agent copy that misses one fails silently.
...
def run_clinicaltrials(args: argparse.Namespace) -> None:
    ...
    if not args.dry_run:
        logger.info("Persisted %d rows",
                    persist_records(records, "trial_protocols", milvus_uri=_milvus_uri()))
