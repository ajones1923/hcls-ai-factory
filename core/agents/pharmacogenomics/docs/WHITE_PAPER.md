...
### 8.2 Three-Engine Platform Architecture

The PGx Intelligence Agent operates within the HCLS AI Factory three-engine precision medicine platform:

1. **Genomics Engine:** Parabricks/DeepVariant/BWA-MEM2 produce annotated VCF files consumed by the star allele calling pipeline
2. **RAG/Chat Engine:** Shared Milvus infrastructure (3.56M vectors) provides the genomic_evidence collection for cross-referencing variants
3. **Drug Discovery Engine:** BioNeMo MolMIM/DiffDock/RDKit generate drug candidates whose metabolism profiles are evaluated by the PGx agent

The agent coordinates with 4 peer agents via HTTP REST: the Oncology Agent for chemotherapy PGx (DPYD, UGT1A1, TPMT), the Cardiology Agent for anticoagulant PGx (warfarin, clopidogrel), the Neurology Agent for antiepileptic PGx (phenytoin, carbamazepine), and the Clinical Trial Agent for PGx-stratified trial eligibility.

### 8.2a Pediatric Oncology PGx

The agent addresses a critical gap in pediatric oncology pharmacogenomics:

- **TPMT/NUDT15 for 6-MP in ALL:** Mandatory genotyping before 6-mercaptopurine maintenance therapy; PM requires 90% dose reduction to prevent fatal myelosuppression
- **CYP3A5 for Vincristine:** CYP3A5 expressers show increased clearance during ALL induction/consolidation
- **UGT1A1 for Irinotecan:** UGT1A1*28 homozygotes at high risk for severe toxicity in pediatric solid tumors; 30% dose reduction recommended
- **MTHFR for Methotrexate:** C677T/A1298C variants modulate methotrexate toxicity risk during high-dose ALL protocols

### 8.3 Edge Deployment Advantage
...
9. **Pediatric PGx modules** -- Age-adjusted dosing algorithms accounting for developmental pharmacogenomics (ontogeny of CYP enzymes, age-dependent allele expression). Age-stratified dosing tables would incorporate CYP enzyme maturation data (e.g., CYP2D6 reaches adult levels by age 5, CYP1A2 by age 3-4) alongside genotype-based adjustments. Initial pediatric oncology focus includes TPMT/NUDT15 for 6-MP in ALL maintenance therapy, CYP3A5 for vincristine clearance, UGT1A1 for irinotecan toxicity in pediatric solid tumors, and MTHFR for high-dose methotrexate monitoring.
