...
The Pharmacogenomics Intelligence Agent operates within the HCLS AI Factory, a three-engine precision medicine platform on NVIDIA DGX Spark:

- **Stage 1 -- Genomics Engine:** Parabricks/DeepVariant/BWA-MEM2 for FASTQ-to-VCF variant calling (GPU-accelerated, 120-240 min)
...
| 6 | **Pharmacogenomics Intelligence** | **:8107** | **Drug metabolism and dosing (this agent)** |
...
### 16.1 Direct Integration (4 Agents)

```
PGx Intelligence Agent (:8107)
       |
       +---> Oncology Agent (:8527)       [Chemotherapy PGx: DPYD, UGT1A1, TPMT dosing]
       |
       +---> Cardiology Agent (:8126)     [Anticoagulant PGx: CYP2C9/VKORC1 warfarin, CYP2C19 clopidogrel]
       |
       +---> Neurology Agent              [Antiepileptic PGx: CYP2C9 phenytoin, HLA-B*15:02 carbamazepine]
       |
       +---> Clinical Trial Agent (:8538) [PGx-guided trial eligibility and stratification]
```

### 16.2 Cross-Agent Trigger Conditions

| Peer Agent | Trigger | PGx Action |
|---|---|---|
| Oncology | Chemotherapy regimen ordered | DPYD fluoropyrimidine screening, UGT1A1 irinotecan, TPMT/NUDT15 thiopurine dosing |
| Cardiology | Anticoagulant/antiplatelet prescribed | CYP2C9/VKORC1 warfarin dosing, CYP2C19 clopidogrel metabolizer status |
| Neurology | Antiepileptic prescribed | HLA-B*15:02 screening for carbamazepine/phenytoin SJS/TEN risk, CYP2C9 phenytoin dosing |
| Clinical Trial | PGx-stratified trial matching | Metabolizer phenotype data for trial eligibility, concomitant medication phenoconversion |

### 16.3 Pediatric Oncology PGx Applications

The agent provides critical pediatric pharmacogenomic decision support for childhood cancer chemotherapy:

- **TPMT/NUDT15 for 6-Mercaptopurine (6-MP) in ALL:** TPMT and NUDT15 genotyping is mandatory before 6-MP maintenance therapy in pediatric acute lymphoblastic leukemia (ALL). Poor metabolizers require 90% dose reduction to avoid life-threatening myelosuppression. The thiopurine dosing algorithm handles combined TPMT+NUDT15 activity scoring.
- **CYP3A5 for Vincristine:** CYP3A5 expressers (*1 carriers) show increased vincristine clearance, potentially requiring dose adjustment. Relevant for ALL induction and consolidation regimens.
- **UGT1A1 for Irinotecan:** UGT1A1*28 homozygotes (poor metabolizers) are at high risk for severe irinotecan toxicity (diarrhea, neutropenia) in pediatric solid tumors. The dosing algorithm recommends 30% dose reduction.
- **MTHFR for Methotrexate:** MTHFR C677T and A1298C variants affect methotrexate toxicity risk in pediatric ALL; the agent surfaces relevant evidence for high-dose methotrexate monitoring.

Communication uses HTTP REST with 30-second timeout and graceful degradation when peer agents are unavailable.
