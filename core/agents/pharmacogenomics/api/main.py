...
# Load API key from core/engines/precision-intelligence .env if not already set
if not os.environ.get("ANTHROPIC_API_KEY"):
    _env_path = Path(os.environ.get("PGX_RAG_PIPELINE_ROOT", "/app/core/engines/precision-intelligence")) / ".env"
    if _env_path.exists():
...
async def lifespan(app: FastAPI):
    ...
    llm_client = build_service_llm(settings.LLM_MODEL, service="pharmacogenomics")
...
try:  # pragma: no cover - platform lib is optional at import time
    ...
    install_api_key_auth(app, service="pharmacogenomics")
    install_governance(app, service="pharmacogenomics", capability_id="pharmacogenomics-intelligence-agent")
