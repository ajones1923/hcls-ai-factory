"""Automated ingest scheduler for the Pharmacogenomics Intelligence Agent.

Periodically refreshes the PubMed PGx literature and ClinicalTrials.gov
PGx trial collections so the knowledge base stays current without manual
...
if _APSCHEDULER_AVAILABLE:
    ...
    class IngestScheduler:
        """Background scheduler that keeps PGx PubMed and ClinicalTrials.gov fresh.
        ...
        ``pgx_last_ingest_timestamp`` Prometheus gauge.
        ...
        def __init__(
            ...
            Args:
                collection_manager: ``PGxCollectionManager`` instance already
                    connected to Milvus.
        ...
        def start(self) -> None:
            ...
            Both jobs execute on a recurring interval.  To trigger an
            immediate refresh call the private ``_refresh_*`` methods directly.
            """
            self._scheduler.add_job(
                self._refresh_pubmed,
                trigger="interval",
                hours=self.interval_hours,
                id="refresh_pgx_pubmed",
                name="PubMed PGx Literature Refresh",
            ...
            self._scheduler.add_job(
                ...
                trigger="interval",
                hours=self.interval_hours,
                id="refresh_pgx_clinical_trials",
                name="ClinicalTrials.gov PGx Refresh",
                replace_existing=True,
            )
        ...
        def _refresh_pubmed(self) -> None:
            """Run the PubMed ingest pipeline with default PGx query.

            Searches for pharmacogenomics literature using queries like:
            'pharmacogenomics OR pharmacogenetics OR CYP2D6 OR CYP2C19
            OR DPYD OR TPMT OR SLCO1B1 OR HLA-B*57:01 OR genotype-guided dosing'

            Updates the ``pgx_last_ingest_timestamp{source="pubmed"}``
            Prometheus gauge on success.
            """
            from .ingest.literature_parser import PubMedIngestPipeline

            logger.info("Scheduler: starting PGx PubMed refresh")
            start = time.time()
            ...
            try:
                ...
                logger.info(
                    f"Scheduler: PGx PubMed refresh complete -- "
                    f"{count} records in {elapsed:.1f}s"
                )

            except Exception as exc:
                logger.error(f"Scheduler: PGx PubMed refresh failed -- {exc}")

        def _refresh_clinical_trials(self) -> None:
            """Run the ClinicalTrials.gov ingest pipeline for PGx trials.

            Searches for pharmacogenomics-related clinical trials using
            terms like: pharmacogenomics, pharmacogenetics, genotype-guided,
            CYP2D6, CYP2C19, DPYD, HLA screening, precision dosing.

            Updates the ``pgx_last_ingest_timestamp{source="clinical_trials"}``
            Prometheus gauge on success.
            """
            ...
            logger.info("Scheduler: starting PGx ClinicalTrials.gov refresh")
            start = time.time()
            ...
            try:
                ...
                logger.info(
                    f"Scheduler: PGx ClinicalTrials.gov refresh complete -- "
                    f"{count} records in {elapsed:.1f}s"
                )

            except Exception as exc:
                logger.error(
                    f"Scheduler: PGx ClinicalTrials.gov refresh failed -- {exc}"
                )
