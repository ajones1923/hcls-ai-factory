...
| the AI platform Component | CAR-T Agent Role |
|---|---|
| **Object Storage** | Raw files: PubMed XML, ClinicalTrials.gov JSON, seed data JSON |
| **data engine** | Event-driven ingest pipelines (fetch → parse → embed → store) |
| **Vector Database** | 11 Milvus collections (10 owned + 1 read-only) + knowledge graph (25 targets, 8 toxicities, 10 mfg) |
| **retrieval engine** | BGE-small embedding + multi-collection RAG + query expansion |
| **agent engine** | CARTRAGEngine (retrieve → augment → generate) + Streamlit UI |
...
| LLM | Claude Sonnet 4.6 | Anthropic API, claude-sonnet-5 |
