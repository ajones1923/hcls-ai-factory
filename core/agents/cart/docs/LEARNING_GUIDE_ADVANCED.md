...
### 12.4 The intelligent agent Model

The `CARTIntelligenceAgent` class follows the intelligent agent pattern:
...
This pattern maps directly to the AI platform orchestration, where an agent engine receives a task, plans its approach, executes subtasks, reflects on results, and produces output.
...
### 13.5 the AI platform Integration

The the AI platform platform provides:

- agent engine for stateful agent execution
- ModelEngine for serving BGE-small-en-v1.5 at scale
- data engine for Milvus management and data pipelines
