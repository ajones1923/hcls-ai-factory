...
def api_get(path: str, timeout: int = 15) -> Optional[dict]:
    ...
    try:
        resp = requests.get(f"{API_BASE}{path}", headers=auth_headers(service="single-cell"), timeout=timeout)
...
def api_post(path: str, data: dict, timeout: int = 60) -> Optional[dict]:
    ...
    try:
        resp = requests.post(
            ...
            json=data,
            timeout=timeout,
            headers=auth_headers({"Content-Type": "application/json"}, "single-cell"),
        )
        resp.raise_for_status()
        return resp.json()
    except requests.exceptions.ConnectionError:
...
with tab_dashboard:
    ...
    try:
        resp = requests.get(f"{API_BASE}/metrics", headers=auth_headers(service="single-cell"), timeout=10)
