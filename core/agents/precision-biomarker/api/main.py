...
# Load API key: prefer env vars, then core/engines/precision-intelligence .env file
_api_key = (
    os.environ.get("ANTHROPIC_API_KEY")
    or os.environ.get("BIOMARKER_ANTHROPIC_API_KEY")
)
if not _api_key:
    _env_path = PROJECT_ROOT.parent.parent / "core/engines/precision-intelligence" / ".env"
    if _env_path.exists():
...
async def lifespan(app: FastAPI):
    ...
    llm_client = build_service_llm(settings.LLM_MODEL, service="precision-biomarker")
...
try:  # pragma: no cover - platform lib is optional at import time
    ...
    install_api_key_auth(app, service="precision-biomarker")
    install_governance(app, service="precision-biomarker", capability_id="precision-biomarker-agent")
