...
| the AI platform Component | Biomarker Agent Role |
|---|---|
| **Object Storage** | Raw reference JSON files: biomarker definitions, genetic variants, PGx rules, disease trajectories |
| **data engine** | Seed pipeline: JSON → BGE-small embedding → Milvus vector insert |
| **Vector Database** | 14 Milvus collections (13 owned + 1 read-only) + 2 sample patients |
| **retrieval engine** | 9 clinical analysis engines + BGE-small embedding + multi-collection RAG |
| **agent engine** | PrecisionBiomarkerAgent orchestrator + Streamlit UI (8 tabs) + FastAPI REST |
