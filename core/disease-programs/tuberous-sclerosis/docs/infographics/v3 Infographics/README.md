# TSC AI Engine — v3 storyboard (vendor-neutral edition)

> **v3 = v2, made vendor-neutral so the HCLS AI innovation leads.** Identical storytelling, frames,
> and narration as v2 — but the hardware/vendor framing is de-emphasized:
> - **`NVIDIA DGX Spark` → `DGX Spark`** (name kept, vendor prefix dropped).
> - **Quantity emphasis removed** — no more "1× …", "only one …", "on a single small computer". The
>   engine leads; the box is mentioned lightly (e.g. "runs on a DGX Spark", "a compact desktop system").
> - **`NVIDIA Omniverse` → `Omniverse`** (prefix dropped; the rendering workflow is still legible).
> - **`RunPod` → `a cloud GPU`** (third-party provider genericized).
> - **Kept:** the affordability point (~$4,699), `OpenUSD`, `RTX`, `Apache 2.0`, the engine/agent names,
>   and the "construct vs. clinical validity" honesty. (Removed NVIDIA-specific specs like GB10 / 128 GB.)
>
> If you want any of these dialed back (e.g. restore `RunPod` or `NVIDIA Omniverse`), say which and I'll
> regenerate. The image `NN / 18` corner indices are unchanged from v2.

Each of the 18 frames is a **slide**, and each has two things in its file:
