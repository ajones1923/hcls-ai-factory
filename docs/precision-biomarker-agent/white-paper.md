...
1. **Genomics Pipeline.** Parabricks 4.6, DeepVariant, and BWA-MEM2 process raw sequencing data (FASTQ) into annotated variant calls (VCF), achieving 120-240 minute genome processing on GPU versus 24-48 hours on CPU.
...
### 12.4 Sibling Intelligence Agents

The biomarker agent operates alongside four sibling agents, each specializing in a distinct clinical domain:

- **CAR-T Intelligence Agent** -- CAR-T cell therapy development lifecycle intelligence across 11 collections spanning manufacturing, clinical, regulatory, and molecular data
- **Oncology Intelligence Agent** -- Cancer genomics and treatment optimization with variant-level therapeutic matching
- **Imaging Intelligence Agent** -- Medical imaging AI with four NVIDIA NIM microservices (VISTA-3D, MAISI, VILA-M3, Llama-3) and 6 clinical workflows
- **Autoimmune Intelligence Agent** -- Autoimmune disease characterization, monitoring, and treatment optimization
